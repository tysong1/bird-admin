...
<!--        </Toolbar>-->
        ...
        <DataTable ref="dt" :value="logs" :selection.sync="selectedProducts" dataKey="id" :paginator="true" :rows="10" :filters="filters"
                   paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rowsPerPageOptions="[5,10,25]"
                   currentPageReportTemplate="Showing {first} to {last} of {totalRecords} products">
          <template #header>
            <div class="table-header">
              <h5 class="p-m-0">Operator Log</h5>
          ...
          <Column field="operatorUserName" header="operatorUserName" sortable></Column>
          <Column field="changeBefore" header="changeBefore" sortable></Column>
          <Column field="changeAfter" header="changeAfter" sortable></Column>
...
<script>
import OperatorLogService from "@/service/OperatorLogService";
export default {
  data() {
    return {
      logs: null,
  ...
  operatorLogService: null,
  created() {
    this.operatorLogService = new OperatorLogService();
  },
  mounted() {
    this.operatorLogService.list().then(res => this.logs = res.data.data);
  },
  methods: {
    openNew() {
      this.role = {};
    ...
    deleteProduct() {
      this.logs = this.logs.filter(val => val.id !== this.role.id);
    ...
    deleteSelectedProducts() {
      this.logs = this.logs.filter(val => !this.selectedProducts.includes(val));
